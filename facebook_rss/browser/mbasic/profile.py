...
class ProfilePage(BaseProfilePage):
    ...
    async def get_posts(self, full: int = 0, limit: int = 0, as_text: int = 0) -> List[Post]:
        if not full:
            posts_count = 1
        else:
        ...
        for item in range(0, posts_count):
            ...
            if not as_text:
                ...
                text = await post.query_selector(self._post_text_selector)
                title = f"{await text.text_content()[:30]}..."
            else:
